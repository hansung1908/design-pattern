...
- isp (interface segregation principle) : 인터페이스 분리 원칙
  ...
  >- phone 인터페이스에 call, sms, alarm, carculator 기능을 정의할 경우 한 기능만 가져와 사용하는 것이 불가능
  >- 그래서, 각각의 기능을 인터페이스로 분리하여 4개의 인터페이스를 원하는대로 구성 가능
  
- dip (dependency inversion principle) : 의존 역전 원칙
  >- 객체들은 나름대로의 원칙을 갖고 정보를 주고 받아야 한다는 설계 원칙
...
- ### 반복자 패턴 (iterator pattern)
  - 기반이 되는 표현을 노출시키지 않고 연속적으로 객체 요소에 접근하는 방법을 제공
  - 집합 객체를 순회하는 클라이언트 코드를 변경하지 않고 다양한 순회 방법 제공 가능
...
- ### 컴포지트 패턴 (composite pattern)
  - 클라이언트 입장에서 전체 계층이든, 부분 계층이든, 최하위 계층이든 동일하게 인식할 수 있게 만드는 패턴
  - 트리 구조에 종속적, 복잡한 트리구조를 편리하게 사용 가능

- ### 파사드 패턴 (facade pattern)
  - 파사드는 불어로 건물의 외벽 및 정문을 의미
  - 소프트웨어 공학적으로 모든 내부 로직을 파사드를 통해 사용하겠다는 의미
  - 현업에서는 기존 레이어드 아키텍처에서 controller와 service 사이에 facade를 두어 1개의 서비스 레이어는 1개의 레포지토리, 더 나아가서 1개의 엔티티에 대한 로직만 수행